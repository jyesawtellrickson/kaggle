...
def seq_to_list(str_seq):
    ...
    """
    return [int(i) for i in str_seq.split(',')]
...
def seq_to_data(seq, n=4):
    ...
    training examples of set length to support
    the model 'learning' the series.

    :param seq: sequence list
    :param n: min size of training set int
    :return:
    """
    X = []
    y = []
...
def lin_pred(seq, print_= 0):
    ...
    """
    # if small sequence, just guess 1 ## why?
    if len(seq) <= 3:
        return 1, seq[-1:][0]
    # choose size of sequence chunks
    ...
    lmr.fit(X, y)
    # predict next number, round for int
