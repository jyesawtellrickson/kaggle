...
print("Importing pd")
df_all = df_all.drop(df_all.select_dtypes(include=['O']).columns.tolist(),axis=1)
df_train = df_all.iloc[:num_train]
df_test = df_all.iloc[num_train:]
id_test = df_test['id']
y_train = df_train['relevance'].values - 1
...
#
param = {'eta': 0.3, 'silent': 1, 'objective':'reg:linear', 'max_depth': 6, 'eval_metric': 'rmse'}
param['nthread'] = 4
...
cv_error = xgb.cv(param, dtrain, num_round, nfold=5, early_stopping_rounds=30, seed=2016)
print("CV Error: ", cv_error[-1:].values.tolist())
print(cv_error)
#
...
pd.DataFrame({"id": id_test, "relevance": y_pred}).to_csv('../output/submission500.csv',index=False)
